...
function createRootGroup(firstFieldKey: string): QueryGroup {
  return {
    id: generateId('group'),
    type: 'group',
    logic: 'AND',
...
const defaultFirstField = SCHEMAS.find(s => s.id === DEFAULT_SCHEMA_ID)?.fields[0]?.key ?? 'name'

const initialRoot = createRootGroup(defaultFirstField)
...
export const useQueryStore = create<QueryStore>()(
  devtools(
    subscribeWithSelector((set, get) => ({
        ...
        set(
          {
            ...
            root: createRootGroup(firstField),
          },
          false,
          'setActiveSchema'
        )
      ...
      resetQuery: () => {
        const firstField = SCHEMAS.find(s => s.id === DEFAULT_SCHEMA_ID)?.fields[0]?.key ?? 'name'
        set(
          {
            root: createRootGroup(firstField),
            activeSchemaId: DEFAULT_SCHEMA_ID,
